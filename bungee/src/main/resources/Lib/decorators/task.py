...
Note that in BungeeCord, synchronous tasks are not allowed. Thus, any functions from the task manager that involve
...
def _schedule(function, delay, interval, time_unit, function_args):
    ...
    if interval > 0:
        if time_unit is not None:
            return task_manager.scheduleAsyncRepeatingTask(function, delay, interval, time_unit, *function_args)
        else:
...
def async_task(delay=0, interval=0, time_unit=None, *args):
    """
    ...
    :param time_unit: The Java TimeUnit of the delay and interval. See https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/concurrent/TimeUnit.html
    :param args:     Positional arguments to pass to the task function when the task executes.
    """

    def _decorator(function):
        handle = _schedule(function, delay, interval, time_unit, args)
