...
public class PluginMessageManager {
    ...
    public void sendRawMessage(Player player, String channel, byte[] payload) {
        if (!Bukkit.getServer().getMessenger().isOutgoingChannelRegistered(PySpigot.get().getPlugin(), channel))
            Bukkit.getServer().getMessenger().registerOutgoingPluginChannel(PySpigot.get().getPlugin(), channel);

        player.sendPluginMessage(PySpigot.get().getPlugin(), channel, payload);
    }
    ...
    public void unregisterListener(ScriptPluginMessageListener listener) {
        Bukkit.getServer().getMessenger().unregisterIncomingPluginChannel(PySpigot.get().getPlugin(), listener.getChannel(), listener);
